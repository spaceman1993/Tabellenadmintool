...
.controller("TableCtrl", ['$scope', '$http', '$filter', 'dataService', 'userFactory', 'activUser', 'benutzerFactory', function($scope, $http, $filter, dataService, userFactory, activUser, benutzerFactory) {
	...
	$scope.initTableControllerVars = function() {
		...
		benutzerFactory.getUserByName(userName ,function(data){
			if(data === null){
				userName = "";
			}else{
				userName = data.name;
		...
		if( userName === null || userName === ""){
			...
			var createJson = {"name" : "admin", "passwort" : "passwort", "einstellung" : settings, "crypt" : "crypt"}
	...
	$scope.loginBenutzer = function(name, passwort) {
		...
		var loginBenutzerJson = {"name" : name};
		
		if(benutzerFactory.getUserByName(loginBenutzerJson ,function(data){}) === null){
			...
			benutzerFactory.getUserByName(loginBenutzerJson ,function(data){
				activUser.user = data.name;
	...
	$scope.addBenutzer = function(name, passwort) {
		...
		var addBenutzerJson = {"name" : name};
		var settings = new Object();
		
		benutzerFactory.getUserByName(addBenutzerJson ,function(data){
			settings = angular.copy(data.einstellung);
		});
